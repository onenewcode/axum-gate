...
pub trait SecretStorageService {
    ...
    fn store_secret(&self, credentials: Credentials<Uuid>) -> impl Future<Output = Result<bool>>;

    /// Updates the credentials. The `secret` should be plain text, hashing is done by the
    /// storage implementation.
    fn update_secret(&self, credentials: Credentials<Uuid>) -> impl Future<Output = Result<()>>;
